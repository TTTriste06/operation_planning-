...
def init_monthly_fields(main_plan_df: pd.DataFrame, start_date: datetime = None) -> list[str]:
    ...
    所有字段格式统一为 "YYYY-MM月字段名"，初始化为 ""。
    
    返回：
    - forecast_months: 所有识别出的月份列表（例如 "2025-07", "2025-08"...）
    """
    HEADER_TEMPLATE = [
    ...
    for ym in forecast_months:
        for header in HEADER_TEMPLATE:
            colname = f"{ym}月{header}"
            if colname not in main_plan_df.columns:
                main_plan_df[colname] = ""
